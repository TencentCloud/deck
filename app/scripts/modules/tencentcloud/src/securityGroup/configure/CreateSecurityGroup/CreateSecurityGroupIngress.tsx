...
import { SelectInput, FirewallLabels } from '@spinnaker/core';
import { FormikProps } from 'formik';
import { ISecurityGroupUpsertCommand } from './CreateSecurityGroup';
import { ISecurityGroupIngress } from './define';

interface IIngressProps {
  formik: FormikProps<ISecurityGroupUpsertCommand>;
}
interface IIngressState {
  inRules: ISecurityGroupIngress[];
}

export default class Ingress extends React.Component<IIngressProps, IIngressState> {
  ...
  handleAdd(ruleset: ISecurityGroupIngress[]) {
    const defaultInRules: ISecurityGroupIngress = {
      cidrBlock: '',
      action: 'ACCEPT',
      protocol: 'TCP',
      port: 7001,
    };
    ruleset.push(defaultInRules);
