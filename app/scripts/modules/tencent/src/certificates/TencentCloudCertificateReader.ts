...
import { ITencentCloudCertificate } from 'tencent/domain';
...
export class TencentCloudCertificateReader {
  public static listCertificates(): IPromise<{ [accountId: string]: ITencentCloudCertificate[] }> {
    return CertificateReader.listCertificatesByProvider('tencent').then((certificates: ITencentCloudCertificate[]) => {
